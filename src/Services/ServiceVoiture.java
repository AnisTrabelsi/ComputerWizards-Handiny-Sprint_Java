...
Connection con=DataSource.getInstance().getConnection();
  ...
  java.sql.Date date_sql = new java.sql.Date(date_util.getTime());
    try { String req = "INSERT INTO `voiture` (`immatriculation`,`marque`,`modele`,`boite_vitesse`,`kilometrage`,`carburant`,`image_voiture`,`description`,`prix_location`,`date_validation_technique`,`id_user`) VALUES ( ?,?,?,?,?,?,?,?,?,?,?);";
...
public List<Voiture> readAll_Of_user(int id) throws SQLException {
    List<Voiture> list = new ArrayList<>();
    try {
        String req = "SELECT * FROM voiture WHERE id_user = ?";
...
public List<Voiture> readFavorites_Of_user(int id) throws SQLException {
    List<Voiture> list = new ArrayList<>();
    try {
        // Modifier la requête SQL pour sélectionner les voitures favorites de l'utilisateur spécifié
        String req = "SELECT v.* FROM voiture v JOIN favoris_voitures f ON v.id_voiture = f.id_voiture WHERE f.id_user = ?";
        PreparedStatement ps = con.prepareStatement(req);
        
        // Utiliser le paramètre 'id' pour filtrer les résultats
        ps.setInt(1, id);
        
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
            // Créer une nouvelle instance de la classe Voiture pour chaque ligne de la table
            Voiture v = new Voiture(rs.getInt("id_voiture"), rs.getString("immatriculation"), rs.getString("marque"), rs.getString("modele"), rs.getString("boite_vitesse"), rs.getString("kilometrage"), rs.getString("carburant"), rs.getString("image_voiture"), rs.getString("description"), rs.getDouble("prix_location"), rs.getDate("date_validation_technique"));
            list.add(v);
        }
    } catch (SQLException ex) {
...
public Voiture findById(int id) throws SQLException {
    Voiture v = null;
    try {
        String req = "SELECT * FROM `voiture` v JOIN `user` u ON v.id_user = u.id_user WHERE v.id_voiture = " + id + ";";
